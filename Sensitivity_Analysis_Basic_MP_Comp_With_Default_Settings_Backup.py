...
def get_arguments():
    ...
    parser.add_argument('-mn', '--method_name', choices=['mp',],
                        help='Method you are analysing')

    parser.add_argument('--exp_num', choices=['a20', 'a21', 'a22', 'a23'],
...
if __name__ == '__main__':
    ...
    if method_name.lower() in ['occlusion', 'mp']:
        rescale_flag = False # since they are already in range [-1, 1] (for Occlusion) and [0, 1] for MP
    elif method_name.lower() in ['grad', 'inpgrad', 'ig', 'lime', 'smoothgrad']:
        rescale_flag = True

    print(f'For method: {method_name}, Rescale flag is {rescale_flag}')
    ...
    for modelIdx, model_name in enumerate(model_names):
        ...
        temp_var = np.zeros(len(img_filenames), dtype=float)
        modelTime = time.time()
        print(f'Calculation variation for model: {model_name}')
        for imgIdx, img_name in enumerate(img_filenames):
            if len(img_filenames) < 2000:
                print(f'Calculation variation across img: {img_name}, img_number: {imgIdx:04d}')
            ...
            if method_name.lower() == 'lime':
                ...
                if args.exp_num.lower() == 'a01':
                    baseline = [i for i in npy_str_lists if 'sample_count_01000' in i]
                    npy_str_lists = [i for i in npy_str_lists if 'sample_count_01000' not in i]
                    assert num_var-1 == 2, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a02':
                    baseline = [i for i in npy_str_lists if 'explainer_seed_0' in i]
                    npy_str_lists = [i for i in npy_str_lists if 'explainer_seed_0' not in i]
                    assert num_var - 1 == 4, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a03':
                    baseline = [i for i in npy_str_lists if 'background_pixel_000' in i]
                    npy_str_lists = [i for i in npy_str_lists if 'background_pixel_000' not in i]
                    assert num_var - 1 == 3, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a04':
                    baseline = [i for i in npy_str_lists if 'explainer_seed_0' in i]
                    npy_str_lists = [i for i in npy_str_lists if 'explainer_seed_0' not in i]
                    assert num_var - 1 == 4, 'Incorrect input path. Check your code'
                ...
                if args.exp_num.lower() == 'a20':
                    baseline = [i for i in npy_str_lists if 'mT_circ' in i]
                    npy_str_lists = [i for i in npy_str_lists if 'mT_circ' not in i]
                    assert num_var-1 == 2, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a21':
                    baseline = [i for i in npy_str_lists if '_iter_299_' in i]
                    npy_str_lists = [i for i in npy_str_lists if '_iter_299_' not in i]
                    assert num_var - 1 == 3, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a22':
                    baseline = [i for i in npy_str_lists if '_blurR_10_' in i]
                    npy_str_lists = [i for i in npy_str_lists if '_blurR_10_' not in i]
                    assert num_var - 1 == 2, 'Incorrect input path. Check your code'
                elif args.exp_num.lower() == 'a23':
                    baseline = [i for i in npy_str_lists if '_seed_0_' in i]
                    npy_str_lists = [i for i in npy_str_lists if '_seed_0_' not in i]
                    assert num_var - 1 == 4, 'Incorrect input path. Check your code'
            ...
            # heatmap_list.append(np.random.uniform(low=-1, high=1,
            #                                       size=heatmap_list[0].shape).astype(heatmap_list[0].dtype))
            ...
            if np.isnan(scores).any():
                temp_mean[imgIdx] = math.nan
                temp_var[imgIdx] = math.nan
                continue

            mean_dict[model_name].append(np.mean(scores))
            var_dict[model_name].append(np.var(scores))

            temp_mean[imgIdx] = np.mean(scores)
            temp_var[imgIdx] = np.var(scores)

        print(f'Len of samples considered is {len(mean_dict[model_name])}')
        output[model_name].append(np.mean(mean_dict[model_name]))
        output[model_name].append(np.mean(var_dict[model_name]))
        print(f'Mean is {output[model_name][0]}, var is {output[model_name][1]}')
        ...
        if len(img_filenames) >= 10:
            ...
            temp_arr = np.concatenate((np.expand_dims(img_file_numbers, axis=-1),
                                       ...
                                       np.expand_dims(temp_var, axis=-1),
                                       ), axis=-1)
    ...
    if len(img_filenames) >= 10000:
        ...
        temp_arr = np.concatenate((np.asarray(([len(mean_dict['googlenet'])] + output['googlenet'])).reshape(1, -1),
                                   np.asarray(([len(mean_dict['pytorch'])] + output['pytorch'])).reshape(1, -1),
                                   np.asarray(([len(mean_dict['madry'])] + output['madry'])).reshape(1, -1),
                                   ), axis=0)
        np.savetxt(file_handle, temp_arr,
                   fmt='%05d,  %.16f,  %.16f',
                   header='ImCou,  Mean              ,  Var', comments='', )

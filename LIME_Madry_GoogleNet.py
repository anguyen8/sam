...
if __name__ == '__main__':
    ...
    parallel = True
    print(f'Paralle is {parallel}')
    ## #Models
    madry_model = eutils.load_madry_model(arch='madry_googlenet', if_pre=args.if_pre, parallel=parallel)
    ...
    for i, (madry_img, pytorch_img, targ_class, img_path) in enumerate(data_loader):
        ...
        if args.if_save_npy == 1:
            ...
            np.save(os.path.join(out_dir, f'time_{f_time}_heatmaps_{img_name}_{par_name}_madry_googlenet.npy'), madry_heatmap)
        ...
        print(f'Time taken for the batch is {time.time() - batch_time}\n')

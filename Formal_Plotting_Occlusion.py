...
def get_arguments():
    ...
    parser.add_argument('--exp_name', choices=['big', 'small', 'all_big'],
                        help='Experiment number of Occlusion'
                             'big - 5, 17, 29, 41, 53'
                             'small - 5, 6, 7'
                             'all_big = 52, 53, 54',
                        )
...
def sort_fun_selection(method, exp_name):
    if method.lower() == 'occlusion':
        if exp_name.lower() == 'big':
            def sort_fun(elem):
                return int(elem.split('/')[-1].split('_patch_size_')[-1].split('_')[0])
        elif exp_name.lower() == 'small':
            def sort_fun(elem):
                return int(elem.split('/')[-1].split('_patch_size_')[-1].split('_')[0])
        elif exp_name.lower() == 'all_big':
            def sort_fun(elem):
                return int(elem.split('/')[-1].split('_patch_size_')[-1].split('_')[0])
        else:
...
if __name__ == '__main__':
    ...
    for idx, img_name in enumerate(img_filenames):
        ...
        elif args.exp_name.lower() == 'small':
            sort_fun = sort_fun_selection(method_name, args.exp_name.lower())
            assert all(i == 3 for i in npy_len_list), \
                'Something is wrong in reading the .npy files\n'
        elif args.exp_name.lower() == 'all_big':
            sort_fun = sort_fun_selection(method_name, args.exp_name.lower())
            assert all(i == 3 for i in npy_len_list), \
                'Something is wrong in reading the .npy files\n'
        else:
        ...
        row_labels_left = []
        row_labels_right = []
        # col_labels = ['Orig_Img', 'GoogleNet', 'GoogleNet-R', 'ResNet50', 'MadryNet']
        col_labels = []
        ...
        #hog_maps = []
        # ipdb.set_trace()
        #for attr in rNet_heatmaps:
        #    _, hog_image = hog(resize(attr, (224, 224)), orientations=8, pixels_per_cell=(16, 16),
        #            cells_per_block=(1, 1), visualize=True, multichannel=False)
        #    # Rescale histogram for better display
        #    hog_image_rescaled = hog_image/np.max(np.abs(hog_image))
        #    # hog_image_rescaled = exposure.rescale_intensity(hog_image, in_range=(0, 10))
        #    hog_maps.append(hog_image_rescaled)
        #grid.extend(hog_maps)

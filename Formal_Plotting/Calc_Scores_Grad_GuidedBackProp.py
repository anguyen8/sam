...
if __name__ == '__main__':
    ...
    if method_name.lower() == 'grad':
        ...
        smooth_dir = '/home/chirag/grad-cam-pytorch/guided_numpy' ##Guided Backprop results
    else:
        grad_dir = '/home/naman/CS231n/heatmap_tests/Madri/Madri_New/robustness_applications/' \
    ...
    for _, img_name in enumerate(img_filenames):
        ...
        rNet_path = glob.glob(os.path.join(smooth_dir, f'*guided_{img_name}*.npy'))
        rNet_path.sort()
        ...
        rNet_heatmaps = [np.mean(np.load(i), axis=0) for i in rNet_path]
    ...
    mean_list = [np.mean(metric_score_rNet), ]
    std_list = [np.std(metric_score_rNet), ]
    print(f'For method {method_name}: '
          f'rNet_avg_ssim: {mean_list[0]}')
    print(f'For method {method_name}: '
          f'rNet_dev_ssim: {std_list[0]}')

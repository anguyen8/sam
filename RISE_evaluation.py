...
def auc(arr):
    ...
    arr_len = len(arr)
    xx = np.arange(arr_len)/(arr_len-1)
    return metrics.auc(xx, arr)/arr.max()
    # return (arr.sum() - arr[0] / 2 - arr[-1] / 2) / (arr.shape[0] - 1)
    # return (arr.sum() - arr[0] / 2 - arr[-1] / 2) / ((arr.shape[0] - 1)*arr.max())
...
class CausalMetric():
    ...
    def single_run(self, img_tensor, explanation, verbose=0, save_to=None):
        ...
        salient_order = np.flip(np.argsort(explanation.reshape(-1, HW), axis=1), axis=-1) #indices of max-min
        for i in range(n_steps+1):
            pred = self.model(start.cuda())
            true_class_prob = round(pred[0, true_class].item(), 4)
            pr, cl = torch.topk(pred, 2)
            if verbose == 3: ## Save all the plots
                plt.figure()
                # plt.title('{} with prob {:.4f}'.format(init_label, scores[prob_idx]))
                plt.axis('off')
                tensor_imshow(start[0])
                if save_to:
                    eutils.mkdir_p(save_to)
                    plt.savefig(os.path.join(save_to,f'Idx_{i:03d}_Prob_{true_class_prob:.04f}_.pdf'))
                    plt.close()
            ...
            if verbose == 2 or (verbose == 1 and i == n_steps) or (save_to and i == n_steps):
                plt.figure(figsize=(15, 5))
                plt.subplot(131)
                plt.title('{} with prob {:.4f}'.format(init_label, scores[prob_idx]))
                plt.axis('off')
                tensor_imshow(init[0])

...
def _model_loop(args, loop_type, loader, model, opt, epoch, adv, writer):
    ...
    *Internal function* (refer to the train_model and eval_model functions for
    ...
    # mods = [mod for _, mod in model.named_modules()]
    #
    # ## get the name of the model
    # if 'module.model' in names:
    #     idx = names.index('module.model')
    #     arch_name = mods[idx].__class__.__name__
    # else:
    #     print('Cant find the model name')
    #     sys.exit(0)
    #
    # print(arch_name)
    # ####################################
    ...
    for i, (inp, target) in iterator:
        ...
        # print(f'Passing the input through the model')
        # import ipdb;
        # ipdb.set_trace()
        ...
        # ## Added by me
        if loop_type == 'train':
            if args.arch.lower() == 'googlenet':  # GoogleNet
                loss = train_criterion(output[0], target) + \
                       0.3 * train_criterion(output[1], target) + \
                       0.3 * train_criterion(output[2], target)
            else:  # normal  mode

...
def get_arguments():
    ...
    parser.add_argument('-idp', '--image_dir_path', help='Path of the image directory', metavar='DIR')
    ...
    parser.add_argument('--exp_name', choices=['a20', 'a21', 'a22', 'a23'],
    ...
    if args.image_dir_path is None:
        print('Please provide image dir path. Exiting')
        sys.exit(1)
    args.image_dir_path = abs_path(args.image_dir_path)
...
def sort_fun_selection(method, exp_name):
    if method.lower() == 'mp':
        if exp_name.lower() == 'a20':
            ...
            def sort_fun(elem):
                key_val = elem.split('/')[-1].split('_iter_')[-1].split('_')[0]
                try:
                    return int(key_val)
                except:
                    return 0
...
if __name__ == '__main__':
    ...
    input_path = args.image_dir_path
    ...
    temp_img_filenames = [input_path.split('/')[-1], ]
    for idx, img_name in enumerate(temp_img_filenames):
        ...
        npy_len_list = [len(gNet_path), len(mNet_path), len(rNet_path)]


        if args.exp_name.lower() == 'a20':
        ...
        elif args.exp_name.lower() == 'a21':
            sort_fun = sort_fun_selection(method_name, args.exp_name.lower())
            assert all(i == 11 for i in npy_len_list), \
                'Something is wrong in reading the .npy files\n'
        elif args.exp_name.lower() == 'a22':
            sort_fun = sort_fun_selection(method_name, args.exp_name.lower())
            assert all(i ==3 for i in npy_len_list), \
                'Something is wrong in reading the .npy files\n'
        elif args.exp_name.lower() == 'a23':
            sort_fun = sort_fun_selection(method_name, args.exp_name.lower())
        ...
        gNet_path = [gNet_path[i] for i in [0, 1, 4, 7, 10]] #7
        rNet_path = [rNet_path[i] for i in [0, 1, 4, 7, 10]] #7
        mNet_path = [mNet_path[i] for i in [0, 1, 4, 7, 10]] #7
        ...
        row_labels_left = []
        row_labels_right = []
        col_labels = ['Orig_Img', 'Init_Mask', 'Iter_10',
                      # 'Iter_50', #'Iter_100',
                      'Iter_150', #'Iter_200',
                      # 'Iter_250', #'Iter_300',
                      'Iter_300', #'Iter_400',
        ...
        temp_masks = [gNet_heatmaps, rNet_heatmaps, mNet_heatmaps]

        # New template
        grid.append(img)
        grid.extend(rNet_heatmaps)
        eutils.zero_out_plot_multiple_patch_chirag([grid],
                                        out_dir,
                                        row_labels_left=[],
                                        row_labels_right=[],
                                        col_labels=[],
                                        file_name=f'time_{f_time}_resnet_{img_name}.pdf',
                                        dpi=224,
                                        save=args.save,
                                        )
        grid = []
        grid.append(255*np.ones_like(img)-2)
        grid.extend(mNet_heatmaps)
        eutils.zero_out_plot_multiple_patch_chirag([grid],
                                        out_dir,
                                        row_labels_left=[],
                                        row_labels_right=[],
                                        col_labels=[],
                                        file_name=f'time_{f_time}_resnet_r_{img_name}.pdf',
                                        dpi=224,
                                        save=args.save,
                                        )
        # for i in range(3):
        #     eutils.zero_out_plot_multiple_patch([[img] + temp_masks[i]],
        #                                         out_dir,
        #                                         [row_labels_left[i]],
        #                                         row_labels_right,
        #                                         col_labels,
        #                                         file_name=f'time_{f_time}_Plot_MP_{img_name}_model_{model_names[i]}.png',
        #                                         dpi=224,
        #                                         save=args.save,
        #                                        )

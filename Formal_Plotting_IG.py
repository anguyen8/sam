...
if __name__ == '__main__':
    ...
    for idx, img_name in enumerate(img_filenames):
        ...
        gNet_heatmap = np.load(gNet_path[0])
        mNet_heatmap = np.load(mNet_path[0])
        rNet_heatmap = np.load(rNet_path[0])



        grid = [[orig_img, gNet_heatmap, rNet_heatmap, mNet_heatmap]]

...
def get_arguments():
    ...
    parser.add_argument('--baseline', choices=['random', 'zero'],
...
if __name__ == '__main__':
    ...
    # size = [299, 299, 299]
    size = [256, 256, 224]
    preprocessFn = transforms.Compose([transforms.Resize((size[0], size[1])),
    ...
    preprocessing_flag_dict  = {'pytorch': True, 'madry': False, 'googlenet': True}
    ...
    for idx, model_name in enumerate(model_names):
        ...
        model = load_model(arch=model_arg, if_pre=args.if_pre)  # Returns probs

        par_name = f'baseline_{args.baseline}_idx_flag_{args.idx_flag}_start_idx_{args.start_idx}_' \

...
def get_arguments():
    ...
    parser.add_argument('--img_idx', type=int,
                        help='Image index to be plotted',
                        )
    ...
    if args.img_idx == None:
        print('Please provide the idx\nExiting')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    image_indices = [int(i.split('_')[-1]) for i in img_filenames]

    try:
        ii = image_indices.index(args.img_idx)
        img_filenames = [img_filenames[ii]]
    except:
        print(f'Provided image index is not in selected list.\nExiting')
        sys.exit(1)
    ...
    inc_image_indices = [int(i.split('_')[-1]) for i in incorrect_img_list]

    if args.img_idx in inc_image_indices:
        print(f'Given index is in excluded list.\nExiting')
        sys.exit(1)
    ...
    for _, img_name in enumerate(img_filenames):
        ...
        gNet_heatmaps = [i/np.max(np.abs(i)) for i in gNet_heatmaps]
        mNet_heatmaps = [i/np.max(np.abs(i)) for i in mNet_heatmaps]
        mGoogleNet_heatmaps = [i/np.max(np.abs(i)) for i in mGoogleNet_heatmaps]
        rNet_heatmaps = [i/np.max(np.abs(i)) for i in rNet_heatmaps]
        ...
        # New template
        eutils.zero_out_plot_multiple_patch_chirag([grid[0]],
                                        out_dir,
                                        row_labels_left=[],
                                        row_labels_right=[],
                                        col_labels=[],
                                        file_name=f'time_{f_time}_Plot_{img_name}_Clean.pdf',
                                        dpi=224,
                                        save=args.save,
                                        )

        eutils.zero_out_plot_multiple_patch_chirag([grid[1]],
                                        out_dir,
                                        row_labels_left=[],
                                        row_labels_right=[],
                                        col_labels=[],
                                        file_name=f'time_{f_time}_Plot_{img_name}_Noisy.pdf',
                                        dpi=224,
                                        save=args.save,
                                        )

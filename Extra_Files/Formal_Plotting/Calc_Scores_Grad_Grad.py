...
if __name__ == '__main__':
    ...
    for _, img_name in enumerate(img_filenames):
        ...
        gNet_path = glob.glob(os.path.join(grad_dir, f'{path}*model_name_googlenet*.npy'))
        gNet_path.sort()
        ...
        rNet_path = glob.glob(os.path.join(grad_dir, f'{path}*model_name_pytorch*.npy'))
        rNet_path.sort()
        ...
        # else:
        gNet_path = [i for i in gNet_path if '_if_noise_0_' in i]
        mNet_path = [i for i in mNet_path if '_if_noise_0_' in i]
        rNet_path = [i for i in rNet_path if '_if_noise_0_' in i]
        ...
        metric_score_gNet.append(compute_score(gNet_heatmaps[0], mGoogleNet_heatmaps[0],
                                               method_name, metric_name))
        metric_score_rNet.append(compute_score(rNet_heatmaps[0], mNet_heatmaps[0],
                                               method_name, metric_name))

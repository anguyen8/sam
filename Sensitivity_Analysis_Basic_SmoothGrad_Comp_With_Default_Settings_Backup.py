...
def get_arguments():
    ...
    parser.add_argument('--exp_num', choices=['a30', 'a31', 'a32'],
                        help = f'Which experiment of Occlusion'
                               f'a30 - Samples 0, 25, 50'
                               f'a31 - Noise - 0, 0.1, 0.2, 0.3'
                               f'a32 - Patch - 52, 53, 54')
    ...
    for modelIdx, model_name in enumerate(model_names):
        ...
        for imgIdx, img_name in enumerate(img_filenames):
            ...
            if rescale_flag:
                ...
                # heatmap_list = [(2 * (hMap - hMap.min()) / (hMap.max() - hMap.min())) - 1 for hI, hMap in
                #                 enumerate(heatmap_list)]
                #
                # baseline_heatmap = [(2 * (hMap - hMap.min()) / (hMap.max() - hMap.min())) - 1 for hI, hMap in
                #                     enumerate(baseline_heatmap)]

...
def get_arguments():
    ...
    parser.add_argument('-l_bp', '--lime_background_pixel', type=int,
...
class DataProcessing:
    ...
    def __getitem__(self, index):
        ...
        if self.if_noise == 1:
            ...
            img = np.asarray(img)
            img = skimage.util.random_noise(img.copy(), 'gaussian', mean=self.mean, var=self.var)
...
if __name__ == '__main__':
    ...
    for i, (madry_img, pytorch_img, targ_class, img_path) in enumerate(data_loader):
        ...
        if args.if_noise == 1:
            ...
            img = np.asarray(img)
            img = skimage.util.random_noise(img.copy(), 'gaussian', mean=args.mean, var=args.var)
            img = Image.fromarray(np.uint8(img * 255))
